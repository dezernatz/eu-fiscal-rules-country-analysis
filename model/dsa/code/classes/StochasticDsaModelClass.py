...
# ========================================================================================= #
    def __init__(self, 
                ...
                deterministic_data_file='deterministic_data_2025_05.csv',  # Add missing parameter
                ): 
        ...
        # CHANGED: Guard stochastic window against short horizons with explicit warnings
        # If the stochastic start or period exceeds the available horizon, clamp and warn.
        max_start = self.end_year - self.start_year - 1
        if self.stochastic_start > max_start:
            import warnings as _warnings
            _warnings.warn(
                "DSA horizon too short for requested stochastic start; clamping start year. "
                "For robust stochastic criteria, we recommend a horizon of at least 17 years.",
                RuntimeWarning,
            )
            self.stochastic_start = max(1, max_start)
            self.stochastic_start_year = self.start_year + self.stochastic_start
        max_period = max(1, self.projection_period - 1 - self.stochastic_start)
        if self.stochastic_period > max_period:
            import warnings as _warnings
            _warnings.warn(
                "DSA horizon too short for requested stochastic period; clamping period length. "
                "For robust stochastic criteria, we recommend a horizon of at least 17 years.",
                RuntimeWarning,
            )
            self.stochastic_period = max_period
        self.stochastic_end = self.stochastic_start + self.stochastic_period - 1 
    ...
    def _get_shock_data(self):
        ...
        if self.shock_frequency == 'quarterly':
            data_file_path = os.path.join(self._base_dir, 'data', 'InputData', 'stochastic_data_quarterly.csv')
            self.df_shocks = pd.read_csv(data_file_path).set_index('YEAR')
        ...
        if self.shock_frequency == 'annual':
            data_file_path = os.path.join(self._base_dir, 'data', 'InputData', 'stochastic_data_annual.csv')
            self.df_shocks = pd.read_csv(data_file_path).set_index('YEAR')
            self.df_shocks = self.df_shocks.loc[self.df_shocks['COUNTRY'] == self.country]
...
def simulate_deficit_jit(N, stochastic_period, pb_sim, iir_sim, ng_sim, d_sim, ob_sim):
    ...
    def check_spb_path_stochastic(self, spb_path, prob_target=None):
        """
        Check whether a given SPB path meets stochastic DSA criteria.
        ...
        prob_target : float, optional
            Target probability for stochastic criteria. If None, uses model's prob_target.
            
        Returns:
        --------
        dict
            Dictionary containing stochastic criterion results
        """
        ...
        try:
            ...
            self.simulate(N=100000)  # Use 100k simulations for accuracy
            
            # Check debt declines probability using existing method
            prob_declines = self.prob_debt_declines()
            results['debt_declines'] = {
                'probability': prob_declines,
                'criterion_met': prob_declines >= self.prob_target,
                'target_probability': self.prob_target
            }
            
            # Check debt below 60% probability using existing method
            prob_below_60 = self.prob_debt_below_60()
            results['debt_below_60'] = {
                'probability': prob_below_60,
                'criterion_met': prob_below_60 >= self.prob_target,
                'target_probability': self.prob_target
            }
            
            # Overall stochastic criterion (both must be met)
            results['overall'] = {
                'criterion_met': (prob_declines >= self.prob_target and 
                                prob_below_60 >= self.prob_target),
                'debt_declines_met': prob_declines >= self.prob_target,
                'debt_below_60_met': prob_below_60 >= self.prob_target
            }
