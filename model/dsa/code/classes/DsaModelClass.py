...
class DsaModel:
    ...
    def __init__(
        ...
        # Store the user-specified end year for display/output purposes
        self.user_end_year = end_year
        # CHANGED: Make internal horizon equal to user-selected end_year so the model can
        # flexibly solve for any chosen horizon without forcing arrays to 2070.
        # Long-horizon anchors are now clamped to available years with explicit warnings.
        ...
        # Scenario-mode toggle: when True, use growth-driven GDP pre-closure
        # (intended for FS→DSA scenario analysis only; left False for general runs)
        self.growth_driven_preclosure = False
        
        # Safeguard: Ensure growth_driven_preclosure always defaults to False
        self._growth_driven_preclosure_user_set = False
    ...
    def ensure_growth_driven_preclosure_default(self):
        """Ensure growth_driven_preclosure defaults to False if not explicitly set by user."""
        if not hasattr(self, '_growth_driven_preclosure_user_set') or not self._growth_driven_preclosure_user_set:
            self.growth_driven_preclosure = False

    # ------------------------------------------------------------------------------------- #
    # Helper methods for safe indexing and short-horizon warnings
    # ------------------------------------------------------------------------------------- #
    def _safe_idx(self, offset: int):
        """
        Clamp an offset index (e.g., T+10, T+30) to the last available index
        in the current projection horizon to avoid out-of-bounds access.
        """
        return max(0, min(offset, self.projection_period - 1))

    def _warn_short_horizon(self, feature: str, required: str):
        """
        Emit a warning when the time horizon is not long enough for a feature.
        Recommends running the DSA for ≥ 17 years for best results.
        """
        warnings.warn(
            f"DSA horizon too short for {feature} (requires ~{required}). "
            f"Proceeding with clamped anchors. For robust deterministic/stochastic criteria, "
            f"we recommend running a horizon of at least 17 years.",
            RuntimeWarning,
        )
    ...
    def _load_input_data(self):
        ...
        # Go up one level from classes directory to code directory, then to EU_DSA directory
        self._base_dir = os.path.join(current_file_dir, '..', '..') + os.sep
        data_file_path = os.path.join(self._base_dir, 'data', 'InputData', self.deterministic_data_file)
        self.df_deterministic_data = pd.read_csv(data_file_path)
        self.df_deterministic_data = (
            self.df_deterministic_data.loc[self.df_deterministic_data['COUNTRY'] == self.country]
            .set_index('YEAR')
            .iloc[:, 1:]
        )

        # Check if data is available in start year (perform check on available index before reindexing)
        available_years = self.df_deterministic_data.index[self.df_deterministic_data.index >= self.start_year]
        if len(available_years) == 0:
            raise ValueError(f"No data available for {self.country} at or after {self.start_year}.")
        ...
        # Reindex to ensure rows exist for the full internal horizon up to self.end_year
        base_index = sorted(set(self.df_deterministic_data.index.tolist())
                            .union(set(range(self.start_year, self.end_year + 1)))
                            .union({0}))
        self.df_deterministic_data = self.df_deterministic_data.reindex(base_index)
    ...
    def _clean_inflation(self):
        ...
        """
        # Anchors at fixed horizons: T, T+1, T+2 from Ameco; T+10 forward; T+30 long-run
        # Build a horizon-invariant path via interpolation between fixed anchors,
        # then slice naturally by the chosen projection length.

        # First three years directly from data
        pi0 = float(self.df_deterministic_data.loc[self.start_year, 'GDP_DEFLATOR_PCH'])
        pi1 = float(self.df_deterministic_data.loc[self.start_year + 1, 'GDP_DEFLATOR_PCH'])
        pi2 = float(self.df_deterministic_data.loc[self.start_year + 2, 'GDP_DEFLATOR_PCH'])

        # Fixed forward and long-run anchors
        pi_10 = float(self.df_deterministic_data.loc[0, 'FWD_INFL_5Y5Y'])
        pi_30 = 2.0

        # Country-specific adjustments
        if self.country in ['POL', 'ROU']:
            # Increase T+10 target by half of the gap to EA at T+2; long-run 2.5
            ea_t2 = float(self.df_deterministic_data.loc[self.start_year + 2, 'EA_GDP_DEFLATOR_PCH'])
            pi_10 = pi_10 + (pi2 - ea_t2) / 2.0
            pi_30 = 2.5
        elif self.country in ['HUN']:
            pi_30 = 3.0

        # Interpolate across fixed knots [0,1,2,10,30]
        x_knots = np.array([0.0, 1.0, 2.0, 10.0, 30.0], dtype=float)
        y_knots = np.array([pi0, pi1, pi2, pi_10, pi_30], dtype=float)

        x_eval = np.arange(self.projection_period, dtype=float)
        pi_path = np.interp(x_eval, x_knots, y_knots)
    ...
    def _clean_debt_redemption(self):
        ...
        # CHANGED: Clamp T+10 tail to available horizon
        idx10 = self._safe_idx(10)
        if idx10 < 10:
            self._warn_short_horizon(feature="debt redemption anchor at T+10", required="T+10+")
        self.D_share_lt_maturing[idx10:] = self.D_share_lt_mat_avg
    ...
    def _clean_market_rates(self):
        ...
        """
        # Get benchmark rates for first two years
        st0 = float(self.df_deterministic_data.loc[self.start_year, 'INTEREST_RATE_ST'])
        lt0 = float(self.df_deterministic_data.loc[self.start_year, 'INTEREST_RATE_LT'])
        st1 = float(self.df_deterministic_data.loc[self.start_year + 1, 'INTEREST_RATE_ST'])
        lt1 = float(self.df_deterministic_data.loc[self.start_year + 1, 'INTEREST_RATE_LT'])

        # Load 10-year forward rates
        self.fwd_rate_st = float(self.df_deterministic_data.loc[0, 'FWD_RATE_3M10Y'])
        self.fwd_rate_lt = float(self.df_deterministic_data.loc[0, 'FWD_RATE_10Y10Y'])

        # Long-run anchors at T+30 (country-specific)
        if self.country in ['POL', 'ROU']:
            lt_30 = 4.5
        elif self.country in ['HUN']:
            lt_30 = 5.0
        ...
        st_30 = lt_30 * yield_curve_coef

        # Interpolate across fixed knots [0,1,10,30] for both st and lt
        x_knots = np.array([0.0, 1.0, 10.0, 30.0], dtype=float)
        y_knots_st = np.array([st0, st1, self.fwd_rate_st, st_30], dtype=float)
        y_knots_lt = np.array([lt0, lt1, self.fwd_rate_lt, lt_30], dtype=float)

        x_eval = np.arange(self.projection_period, dtype=float)
        i_st_path = np.interp(x_eval, x_knots, y_knots_st)
        i_lt_path = np.interp(x_eval, x_knots, y_knots_lt)

        # Assign paths to baselines
        for t in range(self.projection_period):
            self.i_st_bl[t] = i_st_path[t]
            self.i_lt_bl[t] = i_lt_path[t]

        # Set current values to baselines
    ...
    def _calc_rgdp_pers(self):
        ...
        """
        closure_end = self.adjustment_end + self.fiscal_multiplier_persistence
        if getattr(self, 'growth_driven_preclosure', False):
            # Scenario mode: growth-driven GDP pre-closure, implied output gap via identity
            for t in range(1, self.projection_period):
                if t < self.adjustment_start:
                    # Baseline growth
                    self.rg[t] = self.rg_bl[t]
                    self.rgdp[t] = self.rgdp[t - 1] * (1 + self.rg[t] / 100)
                    self.output_gap[t] = (self.rgdp[t] / self.rgdp_pot[t] - 1) * 100
                elif t <= self.adjustment_end:
                    # Apply multiplier to growth with persistence carryover
                    self.fiscal_multiplier_effect[t] = (
                        self.fiscal_multiplier
                        * ((self.spb_bca[t] - self.spb_bca[t - 1]) - (self.spb_bl[t] - self.spb_bl[t - 1]))
                    )
                    ...
                    carryover = sum(
                        [
                            self.fiscal_multiplier_effect[t - i]
                    ...
                    growth_shock = self.fiscal_multiplier_effect[t] + carryover
                    self.rg[t] = self.rg_bl[t] + growth_shock
                    self.rgdp[t] = self.rgdp[t - 1] * (1 + self.rg[t] / 100)
                    self.output_gap[t] = (self.rgdp[t] / self.rgdp_pot[t] - 1) * 100
                elif t <= closure_end:
                    # Apply original closure behavior (keep as in pers version: effect+persistence against baseline gap)
                    self.fiscal_multiplier_effect[t] = (
                        self.fiscal_multiplier
                        * ((self.spb_bca[t] - self.spb_bca[t - 1]) - (self.spb_bl[t] - self.spb_bl[t - 1]))
                    )
                    ...
                    self.output_gap[t] = self.output_gap[t - 1] - (1 / self.fiscal_multiplier_persistence) * self.output_gap[self.adjustment_end] if t > self.adjustment_end else (self.output_gap_bl[t] - self.fiscal_multiplier_effect[t] - persistence_term)
                    self.rgdp[t] = (self.output_gap[t] / 100 + 1) * self.rgdp_pot[t]
                    self.rg[t] = (self.rgdp[t] - self.rgdp[t - 1]) / self.rgdp[t - 1] * 100
                else:
                    # Gap closed
                    self.output_gap[t] = 0.0
            ...
            for t in range(1, self.projection_period):
                if t < self.adjustment_start:
                    # Pre-adjustment: keep baseline output gap
                    self.output_gap[t] = self.output_gap_bl[t]
                elif t <= closure_end:
                    # During adjustment and persistence window: apply persistence logic
                    self.fiscal_multiplier_effect[t] = (
                        self.fiscal_multiplier
                        * (
                            (self.spb_bca[t] - self.spb_bca[t - 1])
                    ...
                    # Persistence term from previous years' impulses
                    persistence_term = sum(
                        [
                            self.fiscal_multiplier_effect[t - i]
                            * (self.fiscal_multiplier_persistence - i)
                    ...
                    # After closure window: keep output gap closed
                    self.output_gap[t] = 0.0

                # Real GDP level and growth from output gap
                self.rgdp[t] = (self.output_gap[t] / 100 + 1) * self.rgdp_pot[t]
                self.rg[t] = (self.rgdp[t] - self.rgdp[t - 1]) / self.rgdp[t - 1] * 100
    ...
    def _calc_rgdp_ec(self):
        ...
        closure_end = self.adjustment_end + self.fiscal_multiplier_persistence
        if getattr(self, 'growth_driven_preclosure', False):
            # Scenario mode: growth-driven GDP until end of adjustment; then apply standard closure
            for t in range(1, self.projection_period):
                if t < self.adjustment_start:
                    # Baseline growth
                    self.rg[t] = self.rg_bl[t]
                    ...
                    # Apply multiplier as growth shock (no persistence in EC flavor here)
                    self.fiscal_multiplier_effect[t] = (
                        self.fiscal_multiplier
                        * ((self.spb_bca[t] - self.spb_bca[t - 1]) - (self.spb_bl[t] - self.spb_bl[t - 1]))
                    )
                    if hasattr(self, 'fiscal_multiplier_spillover'):
                        self.fiscal_multiplier_effect[t] += self.fiscal_multiplier_spillover[t]
                    self.rg[t] = self.rg_bl[t] + self.fiscal_multiplier_effect[t]
                    self.rgdp[t] = self.rgdp[t - 1] * (1 + self.rg[t] / 100)
                    self.output_gap[t] = (self.rgdp[t] / self.rgdp_pot[t] - 1) * 100
                elif t <= closure_end:
                    # Apply original closure schedule (2/3, 1/3, then 0)
                    if t == self.adjustment_start:
                        # not expected here in EC branch
                        self.output_gap[t] = self.output_gap_bl[t] - self.fiscal_multiplier_effect[t]
                    elif t <= self.adjustment_end:
                        # not expected here either, keep symmetry
                        self.output_gap[t] = (
                            (self.fiscal_multiplier_persistence - 1)
                            / self.fiscal_multiplier_persistence
                            * self.output_gap[t - 1]
                            - self.fiscal_multiplier_effect[t]
                        )
                        ...
                        self.output_gap[t] = (
                            self.output_gap[t - 1]
                            - 1 / self.fiscal_multiplier_persistence * self.output_gap[self.adjustment_end]
                        )
                    self.rgdp[t] = (self.output_gap[t] / 100 + 1) * self.rgdp_pot[t]
                    self.rg[t] = (self.rgdp[t] - self.rgdp[t - 1]) / self.rgdp[t - 1] * 100
                else:
            ...
            # Default behavior (unchanged)
            for t in range(1, self.projection_period):
                # Precompute fiscal multiplier effect changes and spillovers when needed
                if t >= self.adjustment_start:
                    self.fiscal_multiplier_effect[t] = (
                        self.fiscal_multiplier
                        * (
                ...
                if t < self.adjustment_start:
                    ...
                    # Linear closure to zero over the persistence window
                    self.output_gap[t] = (
                        self.output_gap[t - 1]
                        - 1 / self.fiscal_multiplier_persistence * self.output_gap[self.adjustment_end]
                    )
                else:
                    # After closure window: keep gap closed
                    self.output_gap[t] = 0.0
    ...
    def find_spb_deterministic(self, criterion, bounds=(-3.5, 3.5), tol=0.0001):
        """
        Find the primary balance that ensures complience with deterministic criteria
        Uses adaptive bounds that automatically adjust if optimization fails.
        """
        ...
        # Run deterministic optimization with adaptive bounds
        return self._deterministic_optimization_adaptive(criterion=criterion, tol=tol)

    def _deterministic_optimization_adaptive(self, criterion, tol, initial_bounds=(-10, 10), reduction_step=0.5, min_bounds=(-50, 50)):
        """
        Run deterministic optimization with adaptive bounds that automatically adjust if optimization fails.
        """
        # Start with wide bounds and adjust as needed
        current_bounds = list(initial_bounds)
        iteration = 0
        max_iterations = 80  # (10 - (-50)) / 0.5 = 120, but 80 is plenty
        
        while iteration < max_iterations:
            try:
                # Try optimization with current bounds
                result = self._deterministic_optimization(criterion, current_bounds, tol)
                return result
                ...
                if "not satisfied at higher bound" in error_msg:
                    # Reduce upper bound (make it more negative)
                    current_bounds[1] -= reduction_step
                    #print(f"Upper bound too restrictive, adjusting to {current_bounds}")
                    
                elif "not satisfied at lower bound" in error_msg:
                    # Reduce lower bound (make it more negative)
                    current_bounds[0] += reduction_step
                    #print(f"Lower bound too restrictive, adjusting to {current_bounds}")
                    
                else:
                    # Other assertion error, re-raise
                    raise e
                
                # Check if we've hit minimum bounds
                if (current_bounds[0] <= min_bounds[0] or 
                    current_bounds[1] >= min_bounds[1]):
                    raise ValueError(
                        f"Could not find valid SPB target for criterion '{criterion}' "
                        f"even with bounds {min_bounds}. The fiscal adjustment required "
                        f"may be beyond reasonable limits. Last attempted bounds: {current_bounds}"
                    )
        
        # Should never reach here due to max_iterations check above
        raise ValueError(f"Adaptive bounds algorithm exceeded maximum iterations ({max_iterations})")
    ...
    def _debt_decline_criterion(self):
        ...
        """
        # CHANGED: Make the window horizon-aware and warn if too short
        end_idx = min(self.adjustment_end + 10, self.projection_period - 1)
        if end_idx <= self.adjustment_end:
            self._warn_short_horizon(feature="debt-decline deterministic criterion (10y window)", required="adjustment_end + 10y")
            # Fallback: evaluate using the last available point only
            return (self.d[self.adjustment_end] <= 60)
        window = self.d[self.adjustment_end:end_idx + 1]
        return (np.all(np.diff(window) < 0) or self.d[end_idx] <= 60)
    ...
    def _deficit_reduction_criterion(self):
        ...
        end_idx = min(self.adjustment_end + 10, self.projection_period - 1)
        if end_idx <= self.adjustment_end:
            self._warn_short_horizon(feature="deficit-reduction deterministic criterion (10y window)", required="adjustment_end + 10y")
            # Fallback: evaluate at last available point
            return (self.ob[self.adjustment_end] >= -3)
        window = self.ob[self.adjustment_end:end_idx + 1]
        return np.all(window >= -3)
    ...
    def df(self, *vars, all=False):
        ...
        # Use user-specified end year for display/output while keeping internal arrays up to 2070
        display_end_year = getattr(self, 'user_end_year', self.end_year)
        display_len = display_end_year - self.start_year + 1
        trimmed = {vars[i]: var[:display_len] for i, var in enumerate(var_values)}
        df = pd.DataFrame(trimmed, index=range(self.start_year, display_end_year + 1))
    ...
    def check_spb_path(self, spb_path, scenarios=None):
        """
        Check whether a given SPB path meets deterministic DSA criteria.
        ...
        scenarios : list of str, optional
            ...
            Options: ['main_adjustment', 'lower_spb', 'financial_stress', 'adverse_r_g', 'deficit_reduction', 'debt_safeguard']
            
        Returns:
        --------
        dict
            Dictionary containing results for each criterion checked
        """
        # Convert input to numpy array
        spb_path = np.array(spb_path)
        
        # Validate input
        if len(spb_path) != self.adjustment_period:
            raise ValueError(f"SPB path must have {self.adjustment_period} values for the adjustment period")
        
        # Set default scenarios if none specified
        if scenarios is None:
            scenarios = ['main_adjustment', 'lower_spb', 'financial_stress', 'adverse_r_g', 'deficit_reduction', 'debt_safeguard']
        
        results = {
            'country': self.country,
            'spb_path': spb_path.tolist(),
            'adjustment_period': self.adjustment_period,
            'adjustment_start_year': self.adjustment_start_year
        }
        ...
        for scenario in scenarios:
            try:
                # Set the SPB path in the model
                self._set_spb_path(spb_path)
                
                # Run projection for the specific scenario
                if scenario == 'main_adjustment':
                    self.project(spb_target=spb_path[-1], scenario=None)
                else:
                    self.project(spb_target=spb_path[-1], scenario=scenario)
                
                # Check the criterion using existing method
                criterion_met = self._deterministic_condition(scenario)
                
                scenario_results[scenario] = {
                    'criterion_met': criterion_met,
                    'debt_end_adjustment': self.d[self.adjustment_end],
                    'debt_10y_after': self.d[min(self.adjustment_end + 10, self.projection_period - 1)],
                    'deficit_end_adjustment': self.ob[self.adjustment_end],
                    'deficit_10y_after': self.ob[min(self.adjustment_end + 10, self.projection_period - 1)]
                }
        ...
        results['deterministic'] = scenario_results
        return results
    
    def _set_spb_path(self, spb_path):
        """Set the SPB path in the DSA model."""
        # Create SPB steps for the adjustment period
        spb_steps = np.diff(spb_path, prepend=self.spb_bca[self.adjustment_start - 1])
        
        # Set the SPB steps in the model
        self.spb_steps = spb_steps
        self.spb_target = spb_path[-1]